...
int main(int argc, char *argv[]) {
	...
	for (param.act_res = param.initial_res; param.act_res <= param.max_res; param.act_res = param.act_res + param.res_step_size) {
		...
		for (iter = 0; iter < param.maxiter; iter++) {
			...
			MPI_Send(lower_border_send, xdim-2, MPI_DOUBLE, lower_rank, 3,
					comm_cart);

			MPI_Recv(upper_border_rec, xdim-2, MPI_DOUBLE, upper_rank, 3,
					comm_cart, MPI_STATUS_IGNORE);

			MPI_Send(upper_border_send, xdim-2, MPI_DOUBLE, upper_rank, 1,
					comm_cart);

			MPI_Recv(lower_border_rec, xdim-2, MPI_DOUBLE, lower_rank, 1,
					comm_cart, MPI_STATUS_IGNORE);

			MPI_Send(left_border_send, ydim-2, MPI_DOUBLE, left_rank, 0,
					comm_cart);

			MPI_Recv(right_border_rec, ydim-2, MPI_DOUBLE, right_rank, 0,
					comm_cart, MPI_STATUS_IGNORE);

			MPI_Send(right_border_send, ydim-2, MPI_DOUBLE, right_rank, 2,
					comm_cart);

			MPI_Recv(left_border_rec, ydim-2, MPI_DOUBLE, left_rank, 2,
					comm_cart, MPI_STATUS_IGNORE);
